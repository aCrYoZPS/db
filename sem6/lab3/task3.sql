CREATE OR REPLACE PROCEDURE generate_sync_ddl(
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('-- Warning: Could not generate DDL for ' || p_type || ' ' || p_name || ': ' ||
                                 SQLERRM);
    END;
...
BEGIN
    ...
    FOR r IN (
        ...
        (SELECT index_name
         ...
         FROM (((SELECT index_name, column_name, column_position
                 ...
                 WHERE index_owner = v_dev)
                MINUS
                (SELECT index_name, column_name, column_position
                 FROM all_ind_columns
                 WHERE index_owner = v_prod))
               UNION
               ((SELECT index_name, column_name, column_position
                 FROM all_ind_columns
                 WHERE index_owner = v_prod)
    ...
    FOR i IN REVERSE 1..v_sorted_objects.COUNT
        LOOP
            DECLARE
        ...
        LOOP
            ...
            FOR col IN (
                ...
                WHERE d.owner = v_dev
                  ...
                  AND d.column_name NOT IN (SELECT column_name
                                            FROM all_tab_columns
                                            WHERE owner = v_prod
                                              AND table_name = r.table_name)
                )
                                    ...
                                    CASE
                                        WHEN col.data_precision IS NOT NULL
                                            THEN '(' || col.data_precision ||
                                                 CASE
                                                     WHEN col.data_scale IS NOT NULL
                                                         THEN ',' || col.data_scale
                                                     ELSE '' END || ')'
                                        ELSE '' END
                                ELSE ''
