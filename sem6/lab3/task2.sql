CREATE OR REPLACE PROCEDURE compare_all_objects(
    ...
    BEGIN
        ...
        IF v_type = 'TABLE' THEN
            FOR r IN (SELECT DISTINCT r.table_name as ref_name
        ...
        ELSE
            FOR r IN (SELECT referenced_type, referenced_name
                ...
                LOOP
                    visit(r.referenced_type || ':' || r.referenced_name);
                END LOOP;
        END IF;
...
BEGIN
    FOR r IN (SELECT table_name
              ...
              FROM (SELECT table_name, column_name, data_type, data_length FROM all_tab_columns WHERE owner = v_dev) dev
                       JOIN (SELECT table_name, column_name, data_type, data_length
                             FROM all_tab_columns
                             WHERE owner = v_prod) prod
    ...
    FOR r IN (SELECT name, type
              ...
              UNION
              SELECT name, type
              FROM (SELECT name, type, line, text
                    FROM all_source
                    WHERE owner = v_dev
                    MINUS
    ...
    FOR r IN (SELECT index_name
        ...
        END LOOP;
    DECLARE
        v_key VARCHAR2(256) := v_status_map.FIRST;
    BEGIN
        WHILE v_key IS NOT NULL
            LOOP
                IF v_status_map(v_key) = 0 THEN visit(v_key); END IF; v_key := v_status_map.NEXT(v_key);
            END LOOP;
    END;
    IF NOT v_cycle_detected THEN
        DBMS_OUTPUT.PUT_LINE('--- SYNC PLAN FOR ' || v_prod || ' ---');
        IF v_sorted_objects.COUNT = 0 THEN
            DBMS_OUTPUT.PUT_LINE('Schemas are identical.');
        ELSE
            FOR i IN 1..v_sorted_objects.COUNT
                LOOP
                    DBMS_OUTPUT.PUT_LINE(i || '. ' || v_sorted_objects(i));
                END LOOP;
        END IF;
    END IF;
END;